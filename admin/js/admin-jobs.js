// FILE: /admin/js/admin-jobs.js (FINAL & CORRECTED)
    ...
    const loadJobs = async (status = 'pending_review') => {
        ...
        try {
            ...
            const q = (status === 'all')
                ...
                : query(jobsRef, where('status', '==', status), orderBy('createdAt', 'desc'));
    ...
    const handleApprove = (jobId) => {
        ...
        document.getElementById('generic-confirm-btn').onclick = async () => {
            try {
                ...
                hideAdminModal();
                loadJobs(document.querySelector('.a-tab-item.active').dataset.status);
            } catch (error) {
