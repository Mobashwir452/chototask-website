...
async function loadDeposits(){
  const q = query(collection(db, 'deposits'), where('status','==','pending'), orderBy('requestedAt','desc'), limit(6));
  const snap = await getDocs(q);
...
async function loadWithdrawals(){
  const q = query(collection(db, 'withdrawals'), where('status','==','pending'), orderBy('requestedAt','desc'), limit(6));
  const snap = await getDocs(q);
